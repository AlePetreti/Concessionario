...
public class FactoryAutomobili {
    ...
    public Automobile creaAutoRandom1() {
        ...
        int[] prezzo = { 30000, 10000, 50000, 80000, 25000, 44000, 37000 };
        int prezzoAuto = prezzo[rnd.nextInt(prezzo.length)];
        return new Automobile(modelloAuto, marcaAuto, 0, numeroPorteAuto, cilindrataAuto, cavalliAuto,
                StatoMacchina.NUOVO, prezzoAuto);
    }
    ...
    public List<Automobile> creaAutoRandom() {
        ...
        for (int i = 0; i < 10; i++) {
            String marcaAuto = marche[rnd.nextInt(marche.length)];
            automobili.add(new Automobile(selezionaModello(marcaAuto), marcaAuto, 0,
                    numeroPorte[rnd.nextInt(numeroPorte.length)], cilindrata[rnd.nextInt(cilindrata.length)],
                    cavalli[rnd.nextInt(cavalli.length)], StatoMacchina.NUOVO, prezzo[rnd.nextInt(prezzo.length)]));
        }
        return automobili;
    }
